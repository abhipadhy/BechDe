...
<body>
 ...
 <div class="right">
    ...
    <div class="form">
      <form method="POST" action="editpdb.php">
        <p>Username :</p>
        ...
        <input type="text" id="input2" required placeholder="Enter Hostel Block Name" name="input4"/>
        <p>University Name :</p>
        ...
        <button type="submit" id="signup">Update Profile</button>
      </form>
    </div>
  </div>
...
<?php
        ...
        if (mysqli_num_rows($result)!=0) {
           ...
           if($row['dp']!=''){
                echo " 
                <script>
                        document.querySelector('#input1').value='$row[name]';
                        document.querySelector('#email').value='$row[email]';
                        document.querySelector('#mob').value='$row[mob]';
                        document.querySelector('#input2').value='$row[hblock]';
                        document.querySelector('#blah').setAttribute('src','$row[dp]');
